...
const TEXT_SPEED: f32 = 10.0;
const TEXT_LIFETIME: f32 = 0.5;

#[derive(Component)]
pub struct BetText(Timer);
...
pub fn bet_effect(
    ...
    if let Ok(transform) = peep_query.get(bet_results.entity) {
        ...
        let text = commands.spawn((
            ...
            BetText(Timer::from_seconds(TEXT_LIFETIME, TimerMode::Once)),
        )).id();
        // Add Black border
...
pub fn move_bet_text(
    mut bet_text: Query<&mut Transform, With<BetText>>,
    time: Res<Time>,
) {
    for mut transform in bet_text.iter_mut() {
        transform.translation = transform.translation + Vec3::new(0.0, TEXT_SPEED * time.delta_secs(), 0.0);
    }
}
...
pub fn despawn_bet_text(
    ...
    for (entity, mut text) in bet_text.iter_mut() {
        if text.0.tick(time.delta()).is_finished() {
